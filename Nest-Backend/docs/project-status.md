...
| Entity | Table | Key Fields |
|--------|-------|------------|
| AuditLog | audit_logs | userId, action, entityType, entityId, oldValues (jsonb), newValues (jsonb), ipAddress, userAgent |
| LoginActivity | login_activities | userId, email, ipAddress, userAgent, status, loginAt |
| SecuritySession | security_sessions | userId, tokenId (unique), ipAddress, userAgent, expiresAt, revokedAt |
| SecurityEvent | security_events | eventType (enum), severity (enum), userId, details (jsonb) |
| PrivacyRequest | privacy_requests | userId, requestType (enum), status (enum), processedAt |
| ConsentRecord | consent_records | userId, consentType (enum), accepted, acceptedAt |
...
LOGIN_SUCCESS, LOGIN_FAILED, ACCOUNT_LOCKED, PASSWORD_CHANGED, PASSWORD_RESET, SESSION_REVOKED, PERMISSION_CHANGED, ROLE_CHANGED, SUSPICIOUS_ACTIVITY, ACCOUNT_DELETED

### Privacy Request Types & Statuses

**Types:** EXPORT_DATA, DELETE_ACCOUNT  
...
Product Management, Category Management, Brand Management, Orders, Returns, Inventory Adjustments, Purchase Orders, Suppliers, Coupons, Promotions, Campaigns, Support Tickets, Finance Transactions, Settlements, Expenses, User Management, Roles & Permissions, System Settings

### Deliverables

- [x] AuditLog Entity
- [x] LoginActivity Entity
- [x] SecuritySession Entity (separate table `security_sessions` to avoid conflict with existing `user_sessions`)
- [x] SecurityEvent Entity
- [x] PrivacyRequest Entity
- [x] ConsentRecord Entity
- [x] AuditLogService (CRUD + findByEntity + log helper)
- [x] SecurityService (logEvent, logLoginAttempt, findEvents, findLoginActivities)
- [x] SessionService (findAll active, revoke)
- [x] PrivacyRequestService (create, findAll, findByUser, findOne, process)
- [x] ConsentService (createOrUpdate, findByUser)
- [x] AdminAuditLogController (3 endpoints)
- [x] AdminSecurityController (4 endpoints)
- [x] AdminPrivacyController (3 endpoints)
- [x] CustomerPrivacyController (3 endpoints)
- [x] SecurityComplianceModule
- [x] Migration Phase22SecurityCompliance (6 tables + 5 enums + indexes)
- [x] Seed permissions (audit.view, security.view, security.manage, privacy.manage)
- [x] Role mappings updated (SUPER_ADMIN — all via Object.values)
- [x] app.module.ts wiring
- [x] data-source.ts wiring
- [x] Zero TypeScript build errors
